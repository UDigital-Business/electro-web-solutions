            ...
            <TabsList className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 h-auto p-2 mb-8 w-full gap-1">
              {subServicios.map((servicio) => (
                <TabsTrigger 
                  key={servicio.id} 
                  value={servicio.id}
                  className="flex flex-col items-center justify-center md:justify-start p-2 h-auto text-xs min-h-[80px] w-full gap-1 md:gap-2 whitespace-normal"
                >
                  <servicio.icon className="h-4 w-4 flex-shrink-0" />
                  <span className="text-center md:text-left leading-tight text-[14px] sm:text-[14px] break-words hyphens-auto max-w-full word-wrap overflow-wrap-anywhere">
                    {servicio.title}
                  </span>
                </TabsTrigger>
              ))}
